...
export class PayRunsService {
  ...
  async generateMonthlyPayRuns(period: string, user: Users) {
    ...
    for (const emp of employees) {
      ...
      switch (emp.typeContrat) {
        case 'FIXE':
          ...
          try {
            heuresTravaillees = await pointageService.calculateWorkedHours(
              emp.id,
              periode.getMonth() + 1,
              periode.getFullYear(),
              user
  ...
  async approvePayRun(payRunId: number, user: Users) {
    const entrepriseId = user.entrepriseId;
    if (!entrepriseId) {
      throw new Error('Entreprise non sélectionnée');
    }
  ...
  async closePayRun(payRunId: number, user: Users) {
    const entrepriseId = user.entrepriseId;
    if (!entrepriseId) {
      throw new Error('Entreprise non sélectionnée');
    }
    ...
    const unpaidPayslips = payRun.payslips.filter((p: any) => p.status !== 'PAYE');
