...
export class EmailService {
  ...
  async sendConfirmationEmail(email: string, code: string, employeeName: string): Promise<void> {
    const mailOptions = {
  ...
  async sendWelcomeEmail(email: string, password: string, qrCode: string, employeeName: string): Promise<void> {
    const mailOptions = {
      from: process.env.SMTP_USER,
      to: email,
